...
class SpoonThumbnail
{
	...
	private $watermark_position = array('top', 'left');
	private $watermark_padding = 0;
	...
	public function parseToFile($filename, $quality = 100, $chmod = 0666)
	{
		...
		{
			// get current dimensions
			$watermarkProperties = @getimagesize($this->watermark);

			// set current dimensions
			$watermarkWidth = (int) $watermarkProperties[0];
			$watermarkHeight = (int) $watermarkProperties[1];
			$watermarkType = (int) $watermarkProperties[2];
			$watermarkMime = (string) $watermarkProperties['mime'];
			...
			// if width is * calc ratio and end height
			if(!$this->width)
				$width = ($this->height / $currentHeight) * $currentWidth;
			else
				$width = $this->width;

			if($this->watermark_position[0] == 'left')
				$dest_x = 0 + $this->watermark_padding;
			else if($this->watermark_position[0] == 'center')
				$dest_x = ($imageWidth*.5) - ($watermarkWidth*.5);
			else if($this->watermark_position[0] == 'right')
				$dest_x = $imageWidth - $watermarkWidth - $this->watermark_padding;

			// if height is * calc ratio and end height
			if(!$this->height)
				$height = ($this->width / $currentWidth) * $currentHeight;
			else
				$height = $this->height;

			if($this->watermark_position[1] == 'top')
				$dest_y = 0 + $this->watermark_padding;
			else if($this->watermark_position[1] == 'center')
				$dest_y = ($imageHeight*.5) - ($watermarkHeight*.5);
			else if($this->watermark_position[1] == 'bottom')
				$dest_y = $imageHeight - $watermarkHeight - $this->watermark_padding;

			$watermark = imagecreatefrompng($this->watermark);
			imagecreatetruecolor($watermarkWidth, $watermarkHeight);
			
			$this->imagecopymerge_alpha($this->image, $watermark, $dest_x, $dest_y, 0, 0, $watermarkWidth, $watermarkHeight, 100);
		}
		...
		if($this->watermark) @imagedestroy($watermark);
	 ...
	 * @param	bool[optional] $on	Should strict-mode be enabled?
	 */
	public function setWatermark($source = false)
	{
	 ...
	 * Set the strict option.
	 *
	 * @param	string $horizontal	Horizontal position
	 * @param	string $vertical	Vertical position
	 */
	public function setWatermarkPosition($horizontal = 'left', $vertical = 'top')
	{
		$this->watermark_position = array($horizontal, $vertical);
	}
	...
	{
		$this->watermark_padding = (int) $padding;
	}
}
