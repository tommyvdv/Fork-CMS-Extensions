...
//\Spoon::dump($resolutions);
	...
	public static function deleteCategoryAllowed($id)
	{
		...
		$hasAlbums = (bool) BackendModel::get('database')->getVar(
		...
		$hasChildren = (bool) BackendModel::get('database')->getVar(
	...
	*/
	...
	public static function deleteImage($ids)
	{
		...
		if(!empty($emptySetIds)) $db->delete('photogallery_sets_images_content', 'set_id IN (' . implode(', ', $emptySetIds) . ')');
		if(!empty($emptySetIds)) $db->delete('photogallery_sets', 'id IN (' . implode(', ', $emptySetIds) . ')');
		if(!empty($emptySetIds)) BackendModel::get('database')->update('photogallery_albums', array('set_id' => null), 'set_id IN(' . implode(',', $emptySetIds) . ')');
	...
	{
		return (array) BackendModel::get('database')->getRecords('SELECT width, height, method, kind FROM photogallery_extras_resolutions WHERE extra_id = ?',array((int) $id));
	}
	...
	{
		// return
		return (int) BackendModel::get('database')->getVar('SELECT MAX(sequence) FROM photogallery_albums');
	}
	...
	{
		// return
		return (int) BackendModel::get('database')->getVar('SELECT MAX(sequence) FROM photogallery_categories');
	}
	...
	{
		// return
		return (int) BackendModel::get('database')->getVar('SELECT MAX(sequence) FROM photogallery_sets_images WHERE set_id = ? LIMIT 1',array($set_id));
	}
	...
	{
		BackendModel::get('database')->update('photogallery_albums', array('hidden' => 'Y'), 'id IN(' . implode(',', $ids) . ')');
	...
	{
		BackendModel::get('database')->update('photogallery_albums', array('hidden' => 'N'), 'id IN(' . implode(',', $ids) . ')');
		return $ids;
	}
	...
	{
		BackendModel::get('database')->update('photogallery_sets_images', array('hidden' => 'Y'), 'id IN(' . implode(',', $ids) . ')');
		return $ids;
	}
	...
	{
		BackendModel::get('database')->update('photogallery_sets_images', array('hidden' => 'N'), 'id IN(' . implode(',', $ids) . ')');
		return $ids;
	}
