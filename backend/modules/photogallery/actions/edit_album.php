...
class BackendPhotogalleryEditAlbum extends BackendBaseActionEdit
{
	...
	private function loadForm()
	{
		...
		$this->meta->setUrlCallback('BackendPhotogalleryModel', 'getURLForAlbum', array($this->record['id']));
	}
	...
	private function loadDataGrid()
	{
		...
		$this->dataGrid->setColumnFunction(array('BackendPhotogalleryHelper', 'getPreviewHTML50x50_crop'), array($this->record['set_id'], $this->getModule(), '[filename]'), 'preview', true);
	...
	private function validateForm()
	{
		...
		if($this->frm->isSubmitted())
		{
			...
			if($this->frm->isCorrect())
			{
				...
				if(is_callable(array('BackendSearchModel', 'editIndex'))) BackendSearchModel::editIndex('photogallery', $item['id'], array('title' => $item['title'], 'text' => $item['text']));
