...
class ApiCall
{
	...
	{
		if($this->apiURL == '') throw new APICallException('Please provide an API URL.');
		...
		if($authenticate)
		{
			...
			// validate
			if($publicKey == '' || $privateKey == '') throw new APICallException('This method ('. $method .') requires authentication, provide a public and private key.');
		...
		// validate XML
		if($xml === false) throw new APICallException('Invalid XML-response.');

		// is error?
		if(!isset($xml['status']) || (string) $xml['status'] != 'ok')
		{
			// is it a response error
			if(isset($xml->error)) throw new APICallException((string) $xml->error);

			// invalid json?
			else throw new APICallException('Invalid XML-response.');
		}
