...
/**
* This edit product action
 *
 * @author Frederik Heyninck <frederik@figure8.be>
 */
class BackendProductsEdit extends BackendBaseActionEdit
{
	/**
	 * Execute the action
	 */
	...
	private function loadForm()
	{
		...
		$this->categories = BackendProductsModel::getCategoriesForDropdown(true);
		...
		$this->meta->setUrlCallback('BackendProductsModel', 'getURLForProduct', array($this->record['id']));
	}
	...
	private function loadDataGrid()
	{
		...
		// create dataGrid
		$this->dataGrid = new BackendDataGridDB(BackendProductsModel::QRY_DATAGRID_BROWSE_IMAGES_FOR_SET, array($this->record['set_id'], $this->id, BL::getWorkingLanguage()));
		...
		// set colum URLs
		$this->dataGrid->setColumnURL('filename', BackendModel::createURLForAction('edit_image') . '&amp;id=[id]&amp;product_id=' . $this->id);

		// set colums hidden
		// $this->dataGrid->setColumnsHidden(array('category_id', 'sequence'));

		// add edit column
		$this->dataGrid->addColumn('edit', null, BL::lbl('Edit'), BackendModel::createURLForAction('edit_image') . '&amp;id=[id]&amp;product_id=' . $this->id, BL::lbl('Edit'));
		
		
		$this->dataGrid->addColumn('preview', SpoonFilter::ucfirst(BL::lbl('Preview')));
		$this->dataGrid->setColumnFunction(array('BackendProductsHelper', 'getPreviewHTML50x50_crop'), array($this->record['set_id'], $this->getModule(), '[filename]'), 'preview', true);
		...
		$this->dataGrid->setAttributes(array('data-action' => "product_images_sequence"));
	...
	protected function parse()
	{
		...
		$this->tpl->assign('add_images_choose_url', BackendModel::createURLForAction('add_images_choose') . '&amp;product_id=' . $this->id);
	}
	...
	private function validateForm()
	{
		...
		if($this->frm->isSubmitted())
		{
			...
			if($this->frm->isCorrect())
			{
				...
				foreach(BackendProductsModel::getExtraIdsForProduct($this->id) as $extra)
				{
				...
				BackendSearchModel::saveIndex('products', $item['id'], array('title' => $item['title'], 'text' => $item['text']));

				// ping
				if(BackendModel::getModuleSetting($this->getModule(), 'ping_services', false)) BackendModel::ping(SITE_URL . BackendModel::getURLForBlock('products', 'detail') . '/' . $this->meta->getURL());
				
				// save the item
			  	$id = BackendProductsModel::updateProduct($item);
				...
				{
					// add
					$categories[] = array('product_id' => $id, 'category_id' => $categoryId, 'sequence' => $categorySequence++);
				}
				

				// update categories
				BackendProductsModel::updateProductCategories($id, $categories);

				BackendProductsModel::updateSetStatistics($this->record['set_id']);
				...
				if($action == 'delete')
				{
					...
					{
						// Delete cronjob
						if(BackendProductsHelper::existsAmazonS3()) BackendAmazonS3Model::deleteCronjobByData($this->URL->getModule(), 's:8:"image_id";i:' . (int) $id . ';');
					}
					
					foreach($ids as $id)
					{
						$image = BackendProductsModel::getImageWithContent($id, $this->id);
					
						// Backend resolutions
						foreach(BackendProductsModel::$backendResolutions as $resolution)
						{
						...
						// Frontend image
						$resolutions = BackendProductsModel::getUniqueExtrasResolutions();
					...
					$deleted = BackendProductsModel::deleteImage($ids);
					...
					foreach($emptySetsAfterDelete as $id)
					{
						...
						if(BackendProductsHelper::existsAmazonS3()) BackendAmazonS3Model::deleteCronjobByFullPathLike($this->URL->getModule(), $cronjob['full_path']);
					...
					// everything is saved, so redirect to the overview
				  	$this->redirect(BackendModel::createURLForAction('edit') . '&report=edited-product&var=' . urlencode($item['title']) . '&id=' . $this->id . '#tabImages');
					
				} 
				elseif($action == 'hide')
				{
					// set new status
					BackendProductsModel::updateImagesHidden($ids);
				...
				{
					// set new status
					BackendProductsModel::updateImagesPublished($ids);
			  	...
			  	// everything is saved, so redirect to the overview
			  	$this->redirect(BackendModel::createURLForAction('edit') . '&report=edited-product&var=' . urlencode($item['title']) . '&id=' . $this->id);
			}
		}
	}
