 ...
 */
class BackendProductsAddWidgetLightbox extends BackendBaseActionAdd
{
	/**
	 * Execute the action
	 */
				...
				if(!$exists)
				{
					foreach(BackendProductsModel::getAllImages() as $image)
					{
						...
						if(SpoonFile::exists($from))
						{
							...
							$cronjob['data'] = serialize(array('set_id' => $image['set_id'], 'image_id' => null, 'delete_local' => true, 'delete_local_in_time' => BackendProductsModel::DELETE_LOCAL_IN_TIME));
							$cronjob['action'] = 'put';
							$cronjob['location'] = 's3';
