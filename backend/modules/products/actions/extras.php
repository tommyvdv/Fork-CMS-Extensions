 ...
 */
class BackendProductsExtras extends BackendBaseActionIndex
{
	...
	{
		// create dataGrid
		$this->dataGrid = new BackendDataGridDB(BackendProductsModel::QRY_DATAGRID_BROWSE_EXTRAS);
		...
		// functions
		$this->dataGrid->setColumnFunction(array('BackendProductsHelper', 'getResolutionsForDataGrid'), array('[id]'), 'resolutions', true);
		
		$this->dataGrid->addColumn('edit', null, BL::getLabel('Edit'), BackendModel::createURLForAction('edit') . '&amp;id=[id]', BL::getLabel('Edit'));

		$this->dataGrid->setColumnFunction(array('BackendProductsHelper', 'getExtraEditURLForKind'), array('[id]','[kind]', '[action]'), 'edit', true);
