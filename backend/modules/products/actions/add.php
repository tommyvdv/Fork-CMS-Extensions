...
/**
* This add product action
 *
 * @author Frederik Heyninck <frederik@figure8.be>
 */

class BackendProductsAdd extends BackendBaseActionAdd
{
	/**
	 * Execute the action
	 */
	...
	private function loadForm()
	{
		...
		$this->meta->setURLCallback('BackendProductsModel', 'getURLForProduct');
	}
	...
	private function validateForm()
	{
		...
		if($this->frm->isSubmitted())
		{
			...
			if($this->frm->isCorrect())
			{
				...
				$item['sequence'] = (int) BackendProductsModel::getSequenceProduct() + 1;
				...
				else
				{
			  	...
			  	// insert the item
			  	$id = BackendProductsModel::insertProduct($item);
				...
				foreach(BackendProductsModel::getAllExtrasWidgets() as $widget)
				{
					$resolutionsLabel = BackendProductsHelper::getResolutionsForExtraLabel($widget['id']);
					...
					$extraId = BackendProductsModel::insertModulesExtraWidget($extra);

					BackendProductsModel::insertExtraId(array('product_id' => $item['id'], 'extra_id' => $widget['id'], 'modules_extra_id' => $extraId));
				}
				...
				if(BackendModel::getModuleSetting($this->getModule(), 'ping_services', false)) BackendModel::ping(SITE_URL . BackendModel::getURLForBlock('products', 'detail') . '/' . $this->meta->getURL());
			  	...
			  	$this->redirect(BackendModel::createURLForAction('add_images_choose') . '&report=added-product&product_id=' . $item['id']);
			}
		}
	}
