 ...
 */
class BackendProductsEditImage extends BackendBaseActionEdit
{
	...
	private function loadForm()
	{
		...
		$this->meta->setUrlCallback('BackendProductsModel', 'getURLForImage', array($this->record['image_content_id']));
	}
	...
	protected function parse()
	{
		...
		$this->tpl->assign('product_id', $this->product_id);

		$this->tpl->assign('previewImageHTML', BackendProductsHelper::getPreviewHTML128x128_crop($this->record['set_id'], $this->getModule(), $this->record['filename']));
	}
	...
	private function validateForm()
	{
		...
		if($this->frm->isSubmitted())
		{
			...
			if($this->frm->isCorrect())
			{
				...
				$content['product_id'] = $this->product_id;
				...
				$item['id'] = $this->id;
			  	...
			  	// update the item
			  	$item['id'] = BackendProductsModel::updateImage($item, $content);

				// Update some statistics
				BackendProductsModel::updateSetStatistics($this->record['set_id']);

			  	// everything is saved, so redirect to the overview
			  	$this->redirect(BackendModel::createURLForAction('edit') . '&report=edited-image&id=' . $content['product_id'] . '&highlight=row-' . $item['id'] . '#tabImages');
			}
		}
	}
