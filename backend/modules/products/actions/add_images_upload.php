 ...
 */
class BackendProductsAddImagesUpload extends BackendBaseActionAdd
{
	...
	private function getData()
	{
		...
		if($this->set_id !== null && !BackendProductsModel::existsSet($this->set_id))
		{
	...
	private function loadForm()
	{
		...
		for($i = 0; $i< BackendProductsModel::MAX_IMAGES_UPLOAD; $i++)
		{
	...
	private function uploadImage($field, $set_id)
	{
		...
		if($this->frm->getField($field)->isFilled())
		{
			...
			$linkedProducts = BackendProductsModel::getProductsLinkedToSet($this->set_id);

			$extension = $this->frm->getField($field)->getExtension();
			$original_filename = $this->frm->getField($field)->getFileName(false);
			
			// Generate a unique filename
			$filename = BackendProductsModel::getFilenameForImage(time() . '_' . $original_filename, $extension) . '.' . $extension;
			...
			$image['sequence'] = BackendProductsModel::getSetImageSequence($set_id) + 1;
			...
			foreach($linkedProducts as &$linkedProduct)
			{
				...
				$meta['url'] = BackendProductsModel::getURLForImage($original_filename, $linkedProduct['language']);

				// add
				$metaData[$linkedProduct['language']] = $meta;

				// build record
				$temp = array();
				$temp['title'] = $original_filename;
				$temp['product_id'] = $linkedProduct['id'];
				$temp['language'] = $linkedProduct['language'];
				$temp['set_id'] = $set_id;
				$temp['created_on'] = BackendModel::getUTCDate();
				$temp['edited_on'] = BackendModel::getUTCDate();

				// add
				$content[$linkedProduct['language']] = $temp;
			}
			...
			foreach(BackendProductsModel::$backendResolutions as $resolution)
			{
				...
				$this->frm->getField($field)->createThumbnail(
					...
					$allowEnlargement, $forceOriginalAspectRatio, BackendProductsModel::IMAGE_QUALITY
				);
			}

			// Do we need to resize the original image or not?
			if(BackendProductsModel::RESIZE_ORIGINAL_IMAGE)
			{
				...
				$this->frm->getField($field)->createThumbnail(
					...
					BackendProductsModel::MAX_ORIGINAL_IMAGE_WIDTH, BackendPhotogalleryModel::MAX_ORIGINAL_IMAGE_HEIGHT,
					false
				);
			}
			else
			...
			$image['id'] = BackendProductsModel::insertImage($image, $content, $metaData);
			...
			$cronjob['data'] = serialize(array('set_id' => $set_id, 'image_id' => $image['id'], 'delete_local' => true, 'delete_local_in_time' => BackendProductsModel::DELETE_LOCAL_IN_TIME));
			$cronjob['action'] = 'put';
			$cronjob['location'] = 's3';
	...
	private function validateForm()
	{
		...
		if($this->frm->isSubmitted())
		{
			...
			// Validate images
			for($i = 0; $i < BackendProductsModel::MAX_IMAGES_UPLOAD; $i++) self::validateImage('image' . $i);
			...
			if($this->frm->isCorrect())
			{
				...
				if($this->filledImagedCount > 0)
				{
					...
					if($this->set_id === null)
					{
						// Create a set based on the product name
						...
						$this->set_id = BackendProductsModel::insertSet($item);

						// Link set to product
						BackendProductsModel::updateProduct(array('id' => $this->id, 'set_id' => $this->set_id));
					}
					
					ini_set('memory_limit', -1);
					
					// upload images
					for($i = 0; $i < BackendProductsModel::MAX_IMAGES_UPLOAD; $i++) self::uploadImage('image' . $i,  $this->set_id);
					
					// Update some statistics
					BackendProductsModel::updateSetStatistics($this->set_id);
	...
	private function validateImage($field)
	{
		if($this->frm->getField($field)->isFilled())
		{
			...
			if($this->frm->getField($field)->isAllowedExtension(array('jpg', 'jpeg', 'gif', 'png'), BL::getError('JPGGIFAndPNGOnly')))
			{
				...
				{
					// image is not larger
					if($this->frm->getField($field)->isFilesize(BackendProductsModel::MAX_ORIGINAL_FILE_SIZE, 'mb', 'smaller', BL::getError('FileSizeIsTooLarge')))
					{
