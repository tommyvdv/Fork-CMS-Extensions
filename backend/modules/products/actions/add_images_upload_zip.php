 ...
 */
class BackendProductsAddImagesUploadZip extends BackendBaseActionAdd
{
	...
	private function processImage($path, $file, $set_id)
	{
		...
		foreach(BackendProductsModel::$backendResolutions as $resolution)
		{
			...
			$thumb->parseToFile(
				...
				BackendProductsModel::IMAGE_QUALITY
			);
		}
		...
		{
			$thumb = new SpoonThumbnail($path . '/' . $file, BackendProductsModel::MAX_ORIGINAL_IMAGE_WIDTH, BackendPhotogalleryModel::MAX_ORIGINAL_IMAGE_HEIGHT);
	...
	private function validateForm()
	{
		...
		if($this->frm->isSubmitted())
		{
			...
			if($this->frm->getField('zip')->isFilled())
			{
				...
				$this->frm->getField('zip')->isFilesize(BackendProductsModel::MAX_ZIP_FILE_SIZE, 'mb', 'smaller', BL::getError('FileSizeIsToLarge'));
			}
