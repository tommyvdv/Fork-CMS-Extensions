 ...
 */
/**
 * In this file we store all generic functions that we will be using to help with the products module
 *
...
class BackendProductsHelper
{
	 ...
	 public static function createAmazonS3Cronjobs($module)
	{
		if(self::existsAmazonS3())
		{
			...
			// Get all images
			$images = BackendProductsModel::getAllImages();
	 ...
	 * @return string
	 */
	public static function getPreviewHTMLForProducts50x50_crop($product_id, $module)
	{
		$result = (array) BackendModel::getDB()->getRecord(
				'SELECT i.filename, a.set_id
				FROM products_products AS a
				LEFT JOIN products_sets_images as i ON i.set_id = a.set_id
				WHERE a.id = ? AND i.hidden = ?	
				ORDER BY i.sequence ASC
				LIMIT 1',
				array((int) $product_id, 'N')
		);
	...
	/**
	 * Format the image count of an product
	 *
	 * @param int $num_images_not_hidden The count of image not hidden
	 * @param int $num_images The count of the total images
	 * @return string
	 */
	public static function getNumImagesForProducts($num_images_not_hidden, $num_images)
	{
	...
	public static function processOriginalImage($path)
	{
		...
		if(!SpoonFile::exists(FRONTEND_FILES_PATH . '/' . $path))
		{
			...
			if(self::existsAmazonS3())
			{
				...
				{
					// Not on amazon
					if(!BackendAmazonS3Model::existsCronjobByFullPath('products', $path))
					{
	...
	{
		$record = BackendModel::getDB()->getRecord('SELECT width, height, method FROM products_extras_resolutions WHERE extra_id = ? AND kind = ? LIMIT 1',array((int) $id, (string) $kind));
	...
	{
		$resolutions = BackendModel::getDB()->getRecords('SELECT width, height, method, kind FROM products_extras_resolutions WHERE extra_id = ? ORDER BY method ASC',array((int) $id));
