...
<div class="pageTitle">
	<h2>{$lblProducts|ucfirst}: {$lblAddImagesForProduct|sprintf:{$record.title}}</h2>
</div>

<div class="wizard">
	<ul>
		<li class="beforeSelected"><a href="{$var|geturl:'edit'}&amp;id={$record.id}"><b><span>1.</span> {$lblWizardInformation|ucfirst}</b></a></li>
		<li class="selected"><a href="{$var|geturl:'add_images_choose'}&amp;product_id={$record.id}"><b><span>2.</span> {$lblWizardAddImages|ucfirst}</b></a></li>
	</ul>
</div>
...
{form:add}
	...
	<div class="box">
		<div class="heading">
			<h3>{$lblExistingProducts|ucfirst}</h3>
		</div>
			<div class="options">
